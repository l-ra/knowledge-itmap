...
function childElements(parent: Element, name?: string): Element[] {
  return Array.from(parent.children).filter((c) => (name ? localName(c) === name : true));
}
 ...
 * Uses DOMParser (browser / happy-dom).
 */
export function parseOpenExchangeXml(xml: string): ExchangeModel {
  ...
  if (err) {
    throw new Error(`Invalid Open Exchange XML: ${err.textContent?.slice(0, 200) || "parse error"}`);
  }
