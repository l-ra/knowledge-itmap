...
export interface SchemaLoadOptions {
  /** Bypass memory + IndexedDB and re-fetch from KC. */
  force?: boolean;
  /** When set, skip network if memory/IDB snapshot matches. */
  fingerprint?: string;
...
export class SchemaResolver {
  ...
  /** Drop memory snapshot (IndexedDB kept unless clearPersisted). */
  clearMemory(): void {
    this.snap = null;
  ...
  /** Clear memory + IndexedDB (e.g. before forced reload / auth change). */
  ...
  private async loadInner(opts: SchemaLoadOptions): Promise<SchemaSnapshot> {
    ...
    if (!force && fp) {
      const cached = await readSchemaCache();
      if (cached && cached.fingerprint === fp) {
        this.snap = hydrateSnapshot(cached);
        this.fingerprint = fp;
        return this.snap;
      }
    }
    ...
    if (fp) {
      await writeSchemaCache(serializeSnapshot(fp, this.snap));
    }
...
export function serializeSnapshot(fingerprint: string, snap: SchemaSnapshot): SchemaCachePayload {
  return {
    version: 1,
    fingerprint,
...
export function hydrateSnapshot(payload: SchemaCachePayload): SchemaSnapshot {
  return {
